...
def evalFedAvgShapley(server, shapley_filter=True, coalition_limit=0):
    ...
    for coalition in coalitions:
        ...
        '''
        for _, metric in args.reward_metrics._get_kwargs():
            if metric == 'LOSS':
                utility_sum += utility_loss
            elif metric == 'ACC':
                utility_sum += utility_acc
    ...
    # Get Federated Average of clients' parameters
    model_state_dicts = [checkpoint['model_state_dict'] for checkpoint in newclient_checkpoints]
    fed_model_state_dict = FedAvg(model_state_dicts)
    ...
    start_time = time.time()  # Time evaluation period
    contributions = {}
    n_coal=len(client_names)
    for client_name in client_names:
        if not client_name in contributions:
           contributions[client_name] = {}
        for coalition in utilities:
            print(coalition)
            if client_name in coalition:
                print(client_name)
                coalition_temp = [client_id for client_id in coalition]
                cur_suborder = coalition_temp
                prev_suborder = copy.deepcopy(coalition_temp)
                prev_suborder.remove(client_name)
                #prev_suborder_i=copy.deepcopy(prev_suborder)
                if len(prev_suborder) != 0:
                   print("not zero")
                   ...
                   S_include_i=len(coalition)
                   weight = (factorial(S_include_i-1)*factorial(n_coal-S_include_i)/factorial(n_coal)) # Weight = |S|!(n-|S|-1)!/n!
                   print(weight)
                   for utility_metric in ans:
                      if utility_metric in contributions[client_name]:
                          print(ans[utility_metric])
                          contributions[client_name][utility_metric] += weight* [ans[utility_metric]]
                      else:
                          contributions[client_name][utility_metric] = [ans[utility_metric]]
                   ...
                   ans = {}
                   for utility_metric in cur_utilities:
                      ans[utility_metric] = max(0, cur_utilities[utility_metric])
                   S_include_i=len(coalition)
                   weight = (factorial(S_include_i-1)*factorial(n_coal-S_include_i)/factorial(n_coal)) # Weight = |S|!(n-|S|-1)!/n!
                   print(weight)
                   for utility_metric in ans:
...
if __name__ == '__main__':
    # parse args
    args = args_parser()
    behaviour_list=list_txt(path='../utils/behavior.txt')
    ...
    NUM_OF_CLIENTS = len(behaviour_list)
    ...
    cli_num=args.clinum
    loadpath=f'../global_Model/global_Model_Server/{args.loadpath}/server/server_model.pt'
    client_filepath1=f'./{args.loadpath}/train_model/'
    client_filepath2=f'../global_Model/global_Model_Server/{args.loadpath}/eval_out/'
    # Initialize system and define helper functions
    createDirectory(client_filepath1)
    createDirectory(client_filepath2)
    # Delete existing .pt files from previous run
    deleteAllModels(client_filepath1)
    deleteAllModels(client_filepath2)
    torch.save(train_dataloaders[cli_num-1], './data/local_train_data.pt')
    if args.train_type == 'naive':
        # Initalize server and clients
        #server = torch.load(loadpath)
        client = initClients(cli_num,behaviour_list,client_filepath1,train_dataloaders)
