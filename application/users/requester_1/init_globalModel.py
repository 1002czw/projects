# -*- coding: utf-8 -*-
"""
Created on Mon Dec  5 15:34:59 2022

@author: czw
"""

# Load libraries
import math, random, copy, os, glob, time, json
...
if __name__ == '__main__':
    ...
    model_path = './global_Model_Server'
    ...
    total_data = sum(len(train_dataset) for train_dataset in train_datasets)
    fraction = [len(train_dataset)/total_data for train_dataset in train_datasets]
    cli_dict={}

    for i in range(NUM_OF_CLIENTS):
        client = f'client_{i+1}'
        cli_dict[client]=fraction[i]
    json_str = json.dumps(cli_dict)

    with open('../utils/basic_model/client_fraction.json', 'w') as json_file:
    ...
    if args.train_type == 'naive':
        ...
        torch.save(test_dataloader, f'{model_path}/FedAvg/server/test_data.pt')
        ...
        torch.save(test_dataloader, f'{model_path}/ShapleyValue/server/test_data.pt')
        ...
        torch.save(test_dataloader, f'{model_path}/GrangerValue/test_data.pt')
