...
func NewSDK() (*fabsdk.FabricSDK, error) {
...
func InstallAndInstantiateCC(sdk *fabsdk.FabricSDK, info *InitInfo) (*channel.Client, error) {
	...
	ccPolicy := cauthdsl.SignedByAnyMember([]string{"org1.iot.com"})
...
func NewChannelCli(sdk *fabsdk.FabricSDK, info *InitInfo) (*channel.Client, error) {
	...
	fmt.Println("可以利用此客户端调用链码进行查询或执行事务.")

	return channelClient, err
