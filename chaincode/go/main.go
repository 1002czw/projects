...
func (t * Iotchaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response{
	...
	} else if functionName == "releaseTaskInfo"{
		return t.releaseTaskInfo(stub,args)
	} else if functionName == "uploadContriInfo"{
		return t.uploadContriInfo(stub,args)
	} else if functionName =="updateContriInfo"{
		return t.updateContriInfo(stub,args)
	}else if functionName =="generateBlock"{
		return t.generateBlock(stub,args)
	}else if functionName =="uploadConsensusMembers"{
		return t.uploadConsensusMembers(stub,args)
	}else {
