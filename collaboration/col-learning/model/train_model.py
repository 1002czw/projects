...
def client_query_lastmodel():
    """return the last model and epoch num"""
    ...
    for f in os.listdir(BASE_DIR):#返回指定的文件夹包含的文件或文件夹的名字的列表
        ...
        if epoch > newest_epoch:
            newest_epoch = epoch-1
            #res_f = f

    # file found
    with open("{}/global_model_{}.ieen".format(BASE_DIR, newest_epoch), 'rb') as rf:
        res = rf.read()
...
pw=new_model["Total_grad_W"]
pb=new_model["Total_grad_b"]
size=new_model["Total_cost"][0]
paw = np.float32(np.array(pw))
pab = np.float32(np.array(pb))
last_model,_=client_query_model()
server_lw,server_lb=last_model
server_W=server_lw-server_lr*paw
server_b=server_lb-server_lr*pab
...
total_cost=new_model["Total_cost"][1]
save2txt(str([int(epoch),test_acc,total_cost]), "testresult.txt")
#print("Epoch: {:03}, cost: {:.2f}, test_acc: {:.4f}".format(epoch, 3.02, test_acc))
'''
plt.figure()
